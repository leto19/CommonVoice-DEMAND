...
Modifications by George Close 2023
"""
...
def nonspeech(wav, sr, vad_aggressiveness=3, webrtc_sr=16_000, webrtc_window_ms=30, dilations=3):
    ...
    # collect all frames with VA, concat into background-audio wave
    # fg = [win for win, is_speech in zip(windows, va) if is_speech]
    return bg


def nonspeech2(wav, sr, vad_aggressiveness=3, webrtc_sr=16_000, webrtc_window_ms=30, dilations=3):
    """
    Args:
        wav: the waveform, a float numpy array with range [-1, 1].
        sr: sample rate of input wav.
    ...
    Returns:
        wave of all regions with no speech content, concatenated together,
        wave of all regions with speech content, concatenated together.
    ...
    # collect all frames with VA, concat into background-audio wave
    fg = [win for win, is_speech in zip(windows, va) if is_speech]
    fg = fg if len(fg) == 0 else np.concatenate(fg)
    return bg,fg
...
def mean_noise_power(wav_fpath):
    ...
    bg = nonspeech(wav, sr,webrtc_sr=sr)

    # compute mean power
    return (bg ** 2).mean()

def mean_noise_power_array(wav,sr):
    # normalize
    wav /= abs(wav).max().clip(1e-6)
