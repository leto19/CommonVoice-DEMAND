...
def find_matching(s,fs,used_list):
    ...
    print(len(other_dur_info))
    if len(other_dur_info) == 0:
        print("No more files left to match!")
        print("Try generating more candidates")
        print("Adios...")
        sys.exit(0)
    s_len = len(s)/fs
...
def simulate(s_name,v_name,snr,used_list):
    ...
    sf.write("%s/test/%s"%(OUTPUT_ROOT,name),y,fs)
    sf.write("%s/test_clean/%s"%(OUTPUT_ROOT,new_name),s,fs)
...
if __name__=='__main__':
    out_dict = {}
    #load all the noise files so we don't have to load them every time  
    living,fs = sf.read("%s/DLIVING/ch01.wav"%DEMAND_ROOT)
    bus,fs = sf.read("%s/TBUS/ch01.wav"%DEMAND_ROOT)
    cafe,fs = sf.read("%s/SCAFE/ch01.wav"%DEMAND_ROOT)
    psquare,fs = sf.read("%s/SPSQUARE/ch01.wav"%DEMAND_ROOT)
    office,fs = sf.read("%s/OOFFICE/ch01.wav"%DEMAND_ROOT)
    used_list = []
    with open("log_testset.csv") as f:
        ...
        for i,row in enumerate(reader):
            ...
            s_path = "%s/clean_testset_wav/%s.wav"%(VOICEBANK_ROOT,name)
            ...
            used_list =  simulate(s_path,v_file,float(snr),used_list)
